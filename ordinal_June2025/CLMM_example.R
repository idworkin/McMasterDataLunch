...
## data are NOT publicly available: please ask Anmol Thind (thinda6 at mcmaster dot ca) for access to data,
##  or me (BMB) for access to the object with the fitted models (44 MB)
...
print(gg_all)
ggsave(gg_all, file = "param_comparison.png")
...
##   that these effects are using *treatment contrasts* (see below)
...
## refit with sum-to-zero contrasts (ugh)
...
tt2 <- tfun(fCRI2) |>  mutate(across(term, ~ reorder(factor(.), estimate)))
gg_zerocontrasts <- gg_final %+% tt2
...
## can get this for individual factors as follows ... but not as convenient overall
emmeans(fCRI, ~PB) |> contrast(method = "eff")
