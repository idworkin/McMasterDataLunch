...
author: "Ian Dworkin"
bibliography: PowerAnalysisDataLunch.bib
...
Figure 1 from @foxEnvironmental2001


## Points to think about before starting your power analysis

* Power analysis to maximize precision of your quantities of interest
* @lakensSample2022 is a good starting place for considering how to determine appropriate/sufficient sample sizes (including a [Shiny app](https://shiny.ieis.tue.nl/sample_size_justification/)!)
...
```{r one_tail_t, echo = FALSE, include = TRUE}
par(las=1)
curve(dt(x, 25), -5, 5, 
      lwd = 2, ylim = c(0, 0.5),
      ylab = "prob",
...
```{r qt_curve, echo = TRUE, eval = FALSE}
...
```{r plot_qt_curve, echo = FALSE,include = TRUE}
...
## `power.t.test`
```{r pow_test2, echo = TRUE}
pow.test <- function(x) {
  pow2 <- power.t.test(delta = x, sd = 2, 
                       sig.level = 0.05,
                       power = 0.8) # We only allow delta to vary.
  return(pow2$n) # Pull out the sample size we need
}
```

## `power.t.test`

```{r sapply-power, echo = TRUE}
power.n <- sapply(delta_vals, pow.test)
```

>- This uses one of the `*apply` functions to repeat the function `pow.test` for each element of the vector `delta_vals`. 
>- Thus for each value in the vector `delta_vals` (from 0.1 to 0.5), it puts this value into `pow.test()` and then returns the estimated `n` (# of observations needed to achieve this power).

## `power.t.test`
```{r plot_power_t, echo = FALSE}
par(las=1)
...
## Similarly, base R has functions for 1-way ANOVA power

`power.anova.test` example

```{r power-anova-1, echo = TRUE}
...
- If you are designing experiments and you think it is likely you are going to use mixed models, the [`simr` package](https://cran.r-project.org/web/packages/simr/index.html) is a good choice to learn (relatively straightforward)
- [`EMSS`](https://cran.r-project.org/web/packages/EMSS/index.html) has useful sample size calculators.

## roll your own with Monte Carlo simulations

>- It is relatively straightforward to put this in a `for` loop to generate power analyses for more complicated designs/analyses
>- Learning how to do simple *Monte Carlo* simulations gives you lots of flexibility
...
```{r mc_power, include = FALSE, echo = FALSE, eval = TRUE}
save_file <- "mcpower.rda"
...
sample_size <- seq(from = 5, to = 50, by = 1)  # Incrementally increasing sample size from 10 to 100
...
if (!file.exists(save_file)) {
  mc_time <- system.time(
    for (k in 1:length(b))  # loop over effect sizes
    {
      if (interactive()) setTxtProgressBar(pb, k)
      b_b <- b[k]
      for (j in 1:length(sample_size))  # loop over sample sizes
      {
        s_s = sample_size[j]
        for (i in 1:N)
        {
  ...
  save(list =c("power.b", "sample_size", "b", "mc_time"), file = save_file)
} else {
  load(save_file)
}
```
...
## Plotting results from a power analysis
```{r surface_plot, echo = FALSE, eval = TRUE, out.width="100%"}
par(mar=c(2.5,3,0,0))
...
## Plotting results from a power analysis
```{r xyplot-setup, echo = FALSE, include = FALSE, eval = TRUE}
...
# xyplot is in the lattice package
```

```{r xyplot, echo = FALSE, eval = TRUE}
